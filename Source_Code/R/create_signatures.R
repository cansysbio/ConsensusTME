createSignatures <- function(saveRdata = F, saveTxt = F, path.save.sigs = '.' ){
  ...
  cell.types <- unique(unlist(lapply(method.genes[names(method.genes) != 'TIMER'], names)))
  names(consensus.all) <- cell.types
  
  for (cell in cell.types) {
    consensus.all[[cell]] <- unique(unlist(lapply(method.genes[names(method.genes) != 'TIMER'],function(x) x[names(x) == cell])))
  }       
  ...
  for (cancer in cancer.types){
     timer.genes <- timer.all[timer.all$Disease.Name == cancer, 1]
     if (cancer == 'Unspecific') {
       cancer.sig <- consensus.all
     } else {
       cancer.sig <- lapply(consensus.all, function(x) intersect(x,timer.genes))
     }
     consensus.cancer[[cancer]] <- cancer.sig
     if (saveTxt) {
       ...
       write.table(sig.df, sprintf('%s/%s_Gene_Signatures.txt', path.txt, cancer),
